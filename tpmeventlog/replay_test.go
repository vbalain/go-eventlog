	...
	for _, bank := range UbuntuAmdSevGCE.Banks {
		...
		for _, cert := range msState.GetSecureBoot().GetDb().GetCerts() {
			...
			case *pb.Certificate_WellKnown:
				if c.WellKnown == pb.WellKnownCertificate_UNKNOWN {
					t.Error(("found WellKnownCertificate_UNKNOWN in db"))
				}
				if c.WellKnown == pb.WellKnownCertificate_MS_THIRD_PARTY_UEFI_CA_2011 {
					contains3PUEFI = true
				} else if c.WellKnown == pb.WellKnownCertificate_MS_WINDOWS_PROD_PCA_2011 {
		...
		if len(pkCerts) != 1 {
			...
			case *pb.Certificate_WellKnown:
				if c.WellKnown != pb.WellKnownCertificate_GCE_DEFAULT_PK {
		...
		if len(kekCerts) != 1 {
			...
			case *pb.Certificate_WellKnown:
				if c.WellKnown != pb.WellKnownCertificate_MS_THIRD_PARTY_KEK_CA_2011 {
...
func TestParseLinuxKernelState(t *testing.T) {
	logs := []struct {
		...
		{COS85AmdSev, "COS85AmdSev", testdata.Cos85AmdSevCmdline},
		{COS93AmdSev, "COS93AmdSev", testdata.Cos93AmdSevCmdline},
		{COS101AmdSev, "COS101AmdSev", testdata.Cos101AmdSevCmdline},
		{Ubuntu2404AmdSevSnp, "Ubuntu2404AmdSevSnp", testdata.Ubuntu2404AmdSevSnpCmdline},
	}
	for _, log := range logs {
		for _, bank := range log.Banks {
			hashName := pb.HashAlgo_name[int32(bank.TCGHashAlgo)]
		...
		for _, bank := range log.Banks {
			...
			t.Run(subtestName, func(t *testing.T) {
				...
				for _, f := range msState.Grub.GetFiles() {
					if bytes.Equal(f.GetUntrustedFilename(), decodeHex(wellknown.EventTagLoadedImageHex)) {
	...
	for _, bank := range eventlog.Banks {
		...
		t.Run(subtestName, func(t *testing.T) {
			...
			if !strings.Contains(err.Error(), "no GRUB measurements found") {
				t.Errorf("expected err (%v) to contain no GRUB measurements error", err)
			}
		})
	}
}
	...
	for _, log := range logs {
		for _, bank := range log.Banks {
			...
			t.Run(subtestName, func(t *testing.T) {
				...
				expectedDigestStrs := log.ExpectedEFIAppDigests[bank.TCGHashAlgo]
				...
				for _, digestStr := range log.ExpectedEFIAppDigests[bank.TCGHashAlgo] {
